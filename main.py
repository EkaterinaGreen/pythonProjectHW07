class Student:
    ...
    def rate_lesson(self, lecturer, course, grade_lecturer):
        if isinstance(lecturer,
                      Lecturer) and course in self.courses_in_progress and course in lecturer.courses_attached and grade_lecturer <= 10:
...
class Lecturer(Mentor):  # лекторы
    ...
    def __str__(self):
        grades_count = 0
        for k in self.grade_lecturer:
            grades_count += len(self.grade_lecturer[k])
        self.average_rating = sum(map(sum, self.grade_lecturer.values())) / grades_count
...
student_3.rate_lesson(best_lecturer_3, 'Python', 7)
